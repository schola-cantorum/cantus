...
This capability governs the `cantus.adapters` package introduced in cantus v0.3.2 (released 2026-05-18). It defines three MVP top-level callables — `expose_as_anthropic_memory_tool` (cantus `Memory` → Anthropic Memory tool spec), `export_as_mcp_server` (cantus `Skill` set → MCP server stdio/HTTP), and `import_mcp_server` (remote MCP tools → cantus `Skill` instances via `_RemoteSkill` closure-based dispatch) — together with the shared adapter contract that all current and future `cantus.adapters` modules SHALL honour: lazy SDK gate with actionable `ImportError("... pip install cantus[<name>]")`, `<adapter>_handshake_failed` / `<adapter>_remote_error` error naming convention, `Skill.spec_for_llm()` JSON shape preservation invariant (`{"name", "description", "args_schema"}` byte-identical with v0.3.0), input-validation hardening (name/version regex, stdio shell-metacharacter rejection, HTTP URL scheme allowlist, port-bind probe), and `is_remote = True` provenance marker that is **not** leaked into `spec_for_llm()`. The design goal is to let cantus Skills and Memories interoperate bidirectionally with industry agent stacks (Anthropic Memory tool / MCP server / MCP client) without breaking the v0.3.0 Skill + Memory dual-kind protocol surface or the v0.3.1 Memory two-tier / Soul / EventStream persistence APIs.
