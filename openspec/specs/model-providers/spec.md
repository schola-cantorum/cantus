...
The factory SHALL lazily import the adapter module only when its prefix is requested. When the corresponding optional dependency is not installed, the factory SHALL raise `ImportError` with an installation hint. For prefixes `"openai"`, `"anthropic"`, `"google"`, `"groq"`, and `"mlx"`, the hint SHALL contain the exact substring `pip install cantus[<provider>]` (each of these has its own dedicated extras dependency closure). For the prefixes `"nvidia"`, `"ollama"`, and `"omlx"`, the hint SHALL contain the exact substring `pip install cantus[openai]` (because these adapters are implemented on top of the OpenAI SDK and route through OpenAI-compatible endpoints — NVIDIA NIM, the local Ollama daemon, and a local omlx / mlx-omni-server respectively; none has its own dedicated extras dependency closure beyond a documentary alias). The `"mlx"` adapter's extras dependency closure (`mlx-lm`) is Apple-Silicon-only; the adapter module surfaces that platform constraint in its own `ImportError`, which the factory SHALL preserve as the exception cause (`from exc`). Additional keyword arguments SHALL pass through to the adapter's constructor (including `api_key` and `base_url`).
...
- **AND** the message names `openai`, `anthropic`, `google`, `groq`, `nvidia`, `ollama`, `mlx`, and `omlx` as the supported prefixes
...
#### Scenario: omlx prefix dispatches to OmlxChatModel

- **WHEN** `load_chat_model("omlx/qwen2.5-coder-7b", base_url="http://localhost:8000/v1")` is called in an environment where the `openai` package is importable
- **THEN** the factory lazily imports `cantus.model.providers.omlx`
- **AND** the returned instance is an `OmlxChatModel` whose `model_id` is `"qwen2.5-coder-7b"`

#### Scenario: Missing optional extras for omlx points at the openai extras group

- **WHEN** `load_chat_model("omlx/qwen2.5-coder-7b", base_url="http://localhost:8000/v1")` is called in an environment where the `openai` package is not installed
- **THEN** the call raises `ImportError`
- **AND** the message contains the substring `pip install cantus[openai]`
- **AND** the message does NOT contain the substring `cantus[omlx]` as a hint that is different from `cantus[openai]`

#### Scenario: mlx prefix dispatches to MLXChatModel

- **WHEN** `load_chat_model("mlx/mlx-community/Mistral-7B-Instruct-v0.3-4bit")` is called in an environment where `mlx_lm` is importable
- **THEN** the factory lazily imports `cantus.model.providers.mlx`
- **AND** the returned instance is an `MLXChatModel` whose `model_id` is `"mlx-community/Mistral-7B-Instruct-v0.3-4bit"`

#### Scenario: Missing optional extras for mlx points at the mlx extras group

- **WHEN** `load_chat_model("mlx/mlx-community/Mistral-7B-Instruct-v0.3-4bit")` is called in an environment where the `mlx_lm` package is not installed
- **THEN** the call raises `ImportError`
- **AND** the message contains the substring `pip install cantus[mlx]`
...
| `omlx/qwen2.5-coder-7b` | `cantus.model.providers.omlx` | `OmlxChatModel` | `pip install cantus[openai]` |
| `mlx/mlx-community/Mistral-7B-Instruct-v0.3-4bit` | `cantus.model.providers.mlx` | `MLXChatModel` | `pip install cantus[mlx]` |

<!-- @trace
source: cantus-local-llm-omlx-server
updated: 2026-06-08
code:
  - cantus/model/factory.py
  - cantus/model/providers/omlx.py
tests:
