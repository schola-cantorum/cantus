...
This capability defines the `Soul` identity contract — how cantus loads agent identity from a host-authored `SOUL.md` file and exposes it to agent runtime code. `Soul.from_file(path)` parses exactly six second-level markdown sections identified by case-sensitive H2 headers (`## Name & Role`, `## Personality`, `## Rules`, `## Tools`, `## Output format`, `## Handoffs`); headers are matched byte-for-byte with no case-folding so that mis-cased headers such as `## name & Role` are surfaced as both missing canonical sections and unexpected vocabulary. Section bodies are captured verbatim with leading and trailing whitespace stripped and assigned to the corresponding attributes on the returned `Soul` instance (`name_and_role`, `personality`, `rules`, `tools`, `output_format`, `handoffs`). Parse failures raise `SoulParseError` with structured `missing_sections` / `duplicates` / `unexpected` lists rather than partial silent success, and a missing file path raises the standard `FileNotFoundError` un-wrapped. The capability deliberately treats SOUL.md content as trusted host-authored input — no escaping, sanitisation, or rewriting — and pushes any validation of untrusted Soul content to host code that reads SOUL.md from end-user uploads.
